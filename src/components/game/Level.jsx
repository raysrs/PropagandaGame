...
function Level({ data }) {
	...
	const lines = data.lines.map((lineData, lineNum) =>
		toComponent(lineData, lineNum)
	);
	...
	function toComponent(lineData, lineNum) {
		if (typeof lineData === 'string') {
			...
			return (
				<SpeechBubble speaker={speaker} key={lineNum}>
					{' '}
					{text}{' '}
				</SpeechBubble>
			);
			...
			return <Question props={lineData} lineNum={lineNum} key={lineNum} />;
		}
	}
	...
	function next() {
		...
		if (
			user.lineNum < lines.length - 1 ||
			data.lines.map((lineData) => typeof lineData).includes('object')
		) {
		...
		<div className='flex h-dvh w-dvw'>
			...
			<div className='flex-auto flex flex-col'>
				...
				<ul className='flex-auto py-2 flex flex-col-reverse overflow-auto'>
					{lines
						.slice(0, user.lineNum + 1)
						.map((line, i) => <li key={i}>{line}</li>)
						.reverse()}
				</ul>
				{
					/* next line button, not rendered if user is prompted or last line of last level */
					!user.prompted &&
						(user.lineNum < lines.length - 1 || user.levelNum < 6) && (
